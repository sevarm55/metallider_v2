...
export default function NewArticlePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const res = await axiosInstance.post("/admin/blog", {
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Как выбрать профильную трубу" />
                </div>
                <div className="space-y-2">
                  <Label>URL (slug)</Label>
                  <Input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="kak-vybrat-profilnuyu-trubu" />
                </div>
                <div className="space-y-2">
                  <Label>Краткое описание (для карточки и SEO)</Label>
                  <Input value={excerpt} onChange={(e) => setExcerpt(e.target.value)} placeholder="Руководство по выбору профильной трубы для строительства" />
                </div>
                <div className="space-y-2">
